...
function checklistLocalKey(userId: string, entradaId: string) {
  return `implemento_checklist:${userId}:${entradaId}:inicio`;
}
...
function pendingChecklistKey(userId: string, entradaId: string) {
  return `implemento_checklist_pending:${userId}:${entradaId}:inicio`;
}
...
function geoCacheKey(userId: string, entradaId: string) {
  return `implemento_geo_cache:${userId}:${entradaId}:inicio`;
}
...
function fotosLocalKey(userId: string, entradaId: string) {
  return `implemento_fotos_local:${userId}:${entradaId}:inicio`;
}
...
function revisionIdLocalKey(userId: string, entradaId: string) {
  return `implemento_revision_id:${userId}:${entradaId}:inicio`;
}
...
const PENDING_PHOTOS_KEY = "implemento_pending_photos_inicio_v1";
...
const PENDING_REVISION_KEY = "implemento_pending_revision_inicio_v1";
...
async function syncPendingData(userId: string, entradaId: string, revisionId: string) {
    ...
    const { error } = await supabase.from("revision_implemento").upsert(
      {
        ...
        tipo: "inicio", timestamp: pending.timestamp,
  ...
  if (pendingCl && Object.keys(pendingCl).length > 0) {
    ...
    else console.error("[sync checklist implemento inicio]", error.message);
  }
  ...
  for (const photo of mine) {
    try {
      ...
      console.error("[sync implemento inicio photo]", e);
      keep.push(photo);
    }
  }
...
function revisionCompleteKey(userId: string, dateISO: string) {
  return `implemento_revision_complete:${userId}:${dateISO}:inicio`;
}
...
export default function OperarioMaquinariaImplementoInicio() {
  ...
  const updateChecklist = async (field: keyof Checklist, value: any) => {
    ...
    if (revisionId && navigator.onLine) {
      const { error } = await supabase
        ...
        .eq("id", revisionId);
      if (error) {
        console.error("[checklist update implemento inicio]", error.message);
  ...
  const handleCapture = async (tipo: FotoTipo) => {
    ...
    try {
      ...
      const filePath = `${userSlug}/implemento/${today}/inicio/${revisionId}/${tipo}.webp`;
      ...
      if (navigator.onLine) {
        ...
        if (uploadErr) {
          console.error("[storage upload implemento inicio]", uploadErr.message);
          ...
          if (upsertErr) console.error("[implemento fotos upsert inicio]", upsertErr.message);
        }
      } else {
        await savePhotoOffline(blob, tipo, filePath, revisionId, user.id, entradaId);
      }
  ...
  return (
    <div className="p-4 max-w-lg mx-auto space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        Captura las <strong>2 fotos obligatorias</strong> y completa el checklist del implemento.
        {isOffline && (
      ...
      <div className="rounded-lg border-2 border-dashed border-muted-foreground/30 p-4 space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold">Revisión Implemento Inicio</h2>
          {checklistCompleto && (
