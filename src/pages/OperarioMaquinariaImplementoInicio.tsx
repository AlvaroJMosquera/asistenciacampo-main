...
export default function OperarioMaquinariaImplementoInicio() {
  ...
  // ── Crear o recuperar revisión implemento INICIO (con soporte offline) ───────
  useEffect(() => {
    if (!user?.id || !entradaId) return;
  ...
  const handleCapture = async (tipo: FotoTipo) => {
    ...
    try {
      ...
      if (navigator.onLine) {
        ...
        if (uploadErr) {
          console.error(
            "[storage upload implemento inicio]",
            uploadErr.message,
          );
          ...
          if (upsertErr)
            console.error(
              "[implemento fotos upsert inicio]",
              upsertErr.message,
            );
        }
      } else {
  ...
  const savePhotoOffline = async (
    ...
    try {
      ...
      console.error("[savePhotoOffline implemento inicio]", e);
    }
  };
    ...
    <div className="p-4 max-w-lg mx-auto space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h1 className="text-xl font-semibold">
          Revisión implemento — inicio turno
        </h1>
        {isOffline && (
      ...
      <p className="text-sm text-muted-foreground">
        Captura las <strong>2 fotos obligatorias</strong> del implemento antes
        de continuar.
        {isOffline && (
