...
export default function OperarioMaquinariaImplementoFin() {
  ...
  // 1) Crear o recuperar revisión implemento FIN
  useEffect(() => {
    if (!user?.id || !entradaId) return;
    ...
    const loadOrCreateRevision = async () => {
      ...
      try {
        ...
        if (selErr) {
          console.error("[revision_implemento fin select]", selErr.message);
          return;
        }
        ...
        if (existing?.[0]?.id) {
          ...
          if (faltaGeo) {
            ...
            if (geoPayload.lat != null && geoPayload.lon != null) {
              const { error: updErr } = await supabase
                ...
                .eq("id", row.id);
              if (updErr) console.error("[revision_implemento update geo fin]", updErr.message);
            }
          }
          ...
          if (fotosExistentes && fotosExistentes.length > 0) {
            ...
            setSubidas((prev) => ({ ...prev, ...next }));
            if (user?.id) writeLocalSubidas(user.id, entradaId, "fin", next);
          }
        ...
        if (insErr) {
          console.error("[revision_implemento fin insert]", insErr.message);
          return;
        }
  ...
  const handleCapture = async (tipo: FotoTipo) => {
    ...
    try {
      ...
      if (upsertErr) {
        console.error("[revision_implemento_fotos upsert fin]", upsertErr.message);
        return;
      }
  ...
  const finalizar = () => {
    ...
    localStorage.setItem(revisionLocalKey(user?.id ?? "", dateISO, "fin"), "1");
